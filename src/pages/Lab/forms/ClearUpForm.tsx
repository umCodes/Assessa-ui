...
import { creditsPerPage } from "../../../credits/credits"

const ClearUpForm = () => {
    const {form, fileInputRef,isLoading, handleFileType, handleSubmit, handleFileRemoval, handleQTypesChange, handleFileUpload, handleFileUploadBtn} = useLabForm();

  return (
    <div>
        <form className="flex flex-col" onSubmit={e => handleSubmit(e, "clear up")}>
            <div className="flex flex-col">
                ...
                <div className="border border-dashed w-fit max-w-[300px] py-2 px-4 mx-auto">
                    ...
                    <p>
                         <span className="max-sm:hidden">Drag and drop or</span> upload your PDF file from your device.
                        <br />
            ...
            <p  className="text-sm text-zinc-600 max-w-sm m-4">
                <FontAwesomeIcon icon={faExclamationCircle}/> This service will clear up specified question types in the uploaded PDF file from their answers. 
                Ensure that the file is well-structured and contains clear questions.
            </p>
            <button
                disabled={isLoading} 
                className={`bg-[#f24c7c] text-white p-1.5 rounded-md text-md font-semibold ml-auto ${isLoading && 'opacity-40'}`}>
                {isLoading ? <FontAwesomeIcon icon={faSpinner} spin/> : 'Clear Up Paper'}
            </button>
        </form>
    </div>
  )
